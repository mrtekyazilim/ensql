...
export function Login() {
  ...
  const getDeviceId = () => {
    let deviceId = localStorage.getItem('partner-deviceId')
    if (!deviceId) {
      deviceId = crypto.randomUUID()
      localStorage.setItem('partner-deviceId', deviceId)
    }
    return deviceId
  }
  ...
  const handlePartnerCodeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Sadece küçük harf, rakam ve tire karakterlerini tut
    value = value.toLowerCase().replace(/[^a-z0-9-]/g, '')
    setPartnerCode(value)

    // Geçersiz karakter girilmişse uyarı göster
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        localStorage.setItem('partnerUser', JSON.stringify(response.data.user))
          ...
          <div className="hidden md:block md:col-span-2 bg-gradient-to-br from-indigo-600 to-purple-700 dark:from-indigo-900 dark:to-purple-950 p-8 relative overflow-hidden">
            ...
            <div className="relative z-10 h-full flex flex-col justify-center">
              <div className="mb-6">
                ...
                <p className="mt-2 text-lg text-indigo-100 font-medium">Partner Panel</p>
              </div>

              <div className="space-y-4 mb-8">
                <p className="text-white text-sm leading-relaxed">
                  Müşterilerinizin raporlarını kolayca yönetin. Kullanıcı tanımlamaları, rapor tasarımları ve müşteri erişimlerini kontrol edin.
                </p>
              </div>
            ...
            <div className="max-w-md mx-auto">
              <div>
                ...
                <p className="mt-2 text-center text-sm text-gray-600 dark:text-gray-400">
                  Partner paneline erişmek için giriş yapın
                </p>
              </div>
              <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    />
                    <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Küçük harf, rakam ve tire (-) kullanabilirsiniz</p>
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="appearance-none relative block w-full px-4 py-3 border border-gray-300 dark:border-gray-600 placeholder-gray-400 dark:placeholder-gray-500 text-gray-900 dark:text-white bg-white dark:bg-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-colors"
                      placeholder="Partner kullanıcı adınızı girin"
                      value={username}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="appearance-none relative block w-full px-4 py-3 border border-gray-300 dark:border-gray-600 placeholder-gray-400 dark:placeholder-gray-500 text-gray-900 dark:text-white bg-white dark:bg-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-colors"
                      placeholder="Partner şifrenizi girin"
                      value={password}
